...
function uses () {
  ...
  var db = [{name: 'Sonic', species: 'Hedgehog'},
        ...
        console.log("In class practice, length of each species value into arr: ")
        console.log(db.map(x => x.species).map(species => species.length));
  console.log(db);
...
/*console.log("moar class practice");
var oldSchool = function (name, age){
  return ({
    name: name,
    age: age,
    selfReport: function() {
...
console.log(
    oldSchool("Carson", 30).selfReport()
    );
